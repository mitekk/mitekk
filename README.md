...
<tr>
  ...
  <td>Two-player, turn-based 8-ball pool you join by short code — break the rack, run your group, sink the 8, with both players watching every shot replay live. <sub>(source private)</sub></td>
...
```mermaid
flowchart LR
    push(["git push"]) --> hook["GitHub App webhook"] --> coolify["Coolify · build + deploy"]
    coolify --> traefik["Traefik · auto-TLS"] --> live["*.mitya.dev"]
    coolify --> pg[("Postgres")]
    coolify --> redis[("Redis")]
